'''Author: Yogesh Verma'''
'''DeepLQ'''
# Arrays are declared to zero values after usage to free the memory allocation during training of DNN
# ! /usr/bin/env python
...
weight_dy = wdy*np.ones(len(DY),dtype=float)
weight_st1 = wst1*np.ones(len(ST_1),dtype=float)
weight_st2 = wst2*np.ones(len(ST_2),dtype=float)
weight_st3 = wst3*np.ones(len(ST_3),dtype=float)
weight_st4 = wst4*np.ones(len(ST_4),dtype=float)
weight_ww = www*np.ones(len(WW),dtype=float)
weight_wz = wwz*np.ones(len(WZ),dtype=float)
weight_zz = wzz*np.ones(len(ZZ),dtype=float)
weight_tt = sigma1*np.ones(len(TT),dtype=float)
weight_50 = sigma2*np.ones(len(LQ50),dtype=float)
weight_80 = sigma3*np.ones(len(LQ80),dtype=float)
weight_110 = sigma4*np.ones(len(LQ110),dtype=float)
weight_140 = sigma5*np.ones(len(LQ140),dtype=float)
weight_170 = sigma6*np.ones(len(LQ170),dtype=float)
weight_200 = sigma7*np.ones(len(LQ200),dtype=float)
weight_230 = sigma8*np.ones(len(LQ230),dtype=float)





# Model definition for Mass regression
...
# Mass training to use additional input
...
#Class definition, Distinct class allocated to each signal and background
...
WZ['class'] = WZ_class
ZZ['class'] = ZZ_class
ST_1['class'] = ST1_class
ST_2['class'] = ST2_class
ST_3['class'] = ST3_class
ST_4['class'] = ST4_class
...
BKG_class = BKG['class'].to_numpy(dtype=float)
BKG = BKG.drop('class', axis=1)
...
# Classification Model Defination
...
def base_model():
    ...
    model.add(LeakyReLU(alpha=leaky_relu_alpha))
    model.add(BatchNormalization())
    model.add(Dense(11,activation='softmax')) #Last node depends on number of classes
...
final_class = np_utils.to_categorical(final_class, 11)
...
df_cm = pd.DataFrame(cm,index = [i for i in ["ttbar","LQ(500)","LQ(800)","LQ(1100)","LQ(1400)","LQ(1700)","LQ(2000)","LQ(2300)","DY","ST","DB"]],columns = [i for i in ["ttbar","LQ(500)","LQ(800)","LQ(1100)","LQ(1400)","LQ(1700)","LQ(2000)","LQ(2300)","DY","ST","DB"]])
